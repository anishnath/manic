...
fn main() {
...

    {
        ...
        s.circle("b", v(920.0, 360.0), 48.0).color(PANEL).label("B").hidden();
        s.arrow("e", v(408.0, 360.0), v(408.0, 360.0)).color(MAGENTA).hidden();
        s.rect("box", v(640.0, 520.0), 120.0, 70.0).outline_color(LIME).hidden();
        s.text("cap", v(640.0, 640.0), "").size(24.0).color(DIM).hidden();
        s.text("title", v(640.0, 200.0), "NEON TERMINAL").display().size(44.0).color(CYAN).hidden();
    }
    ...
    m.play(par![
        ...
        act().grow_to("e", v(872.0, 360.0)).dur(0.6).ease(InOutCubic),
    ]);
    m.play(seq![
