...
SECTIONS = [
 ...
 dict(slug="ml", title="Machine learning", intro=(
   "Small, deterministic models whose displayed values are computed rather than staged. "
   "The ML examples use progressive focus so forward values, supervised loss, reverse "
   "gradients, and parameter updates stay readable on one persistent network."
 ), sub=SUB, items=[
   ("manic-ml-scalar-to-tensor",
    "Start with one value, extend it into a vector, arrange values into a matrix, then "
    "stack channels into a rank-3 tensor. The only ML noun is `tensor`; ordinary Manic "
    "steps, arrows, reveals, and captions tell the complete dimensional story."),
   ("manic-ml-activation-focus",
    "A focused ReLU lesson: draw the truthful activation curve, test one negative and one "
    "positive input, then connect the bend to nonlinearity. `activation` supplies the "
    "mathematics while core Manic owns the probes, guides, equation, and pacing."),
   ("manic-ml-forward-pass",
    "A creator-first neural-network story: introduce the ReLU activation, reveal a seeded "
    "3→6→4→3 model, then follow one real forward pass into softmax probabilities. `network`, "
    "`activation`, and `forward` provide the truthful structure while ordinary named steps, "
    "captions, and Creator branding tell the lesson."),
   ...
   ("manic-ml-cnn-edge-story",
    ...
    "destination cell synchronized while `tensor`, `kernel`, `convolve`, and `pool` supply "
    "the exact values."),
   ("manic-ml-token-embedding",
    "A sentence becomes honest word tokens, stable seeded educational lookup vectors, exact "
    "sinusoidal positions, and model-input vectors. Repeated words prove that token identity "
    "keeps one base embedding while position distinguishes each occurrence."),
   ("manic-ml-transformer-attention",
    "One token finds context through a real scaled dot-product self-attention head. Explicit "
    "embeddings become Q/K/V, one selected softmax row drives the weighted value mix and "
    "residual, and a deterministic educational output projection produces exact top-k "
    "probabilities without pretending to be a pretrained language model."),
   ("manic-ml-transformer-block",
    "One persistent token lane crosses a complete deterministic transformer block: two "
    "causal attention heads, concatenation and output projection, both residual/norm "
    "stages, a GELU MLP, and exact settled outputs. `transformer` owns the computation "
    "while `encode` provides a smooth directly seekable explanation."),
   ("manic-ml-logits-sampling",
    "The same educational LM projection is viewed at low and high temperature before "
    "greedy and top-p decoding make their support explicit. `logits` computes every "
    "candidate from the final hidden row; `sample` filters, renormalizes, and selects "
    "one reproducible next token without pretending to run a pretrained model."),
 ]),
 ...
 dict(slug="systems", title="Diagrams", intro=(
     ...
     "architecture (or a `flowchart`) with auto-positioned nodes and directed connections "
     "— geometry is "
   ...
   ("microservices-platform",
    "Auto-fit showcase: Route 53 → load balancer → gateway, three availability zones of "
    "ECS services, a replicated RDS database cluster, and CloudWatch monitoring — declared "
    "with zero coordinates. Add a tier or a zone and the whole diagram reflows and scales "
    "itself down to the frame; paths coloured by relationship (request · write · "
    "replication · telemetry)."),
   ("factorial-flowchart",
    "A flowchart that runs: `flowchart(fc)` auto-lays seven shape-nodes "
    "(terminator/io/process/decision) top-down with no coordinates, then a token walks the "
    "factorial loop — taking the yes branch, looping back, and exiting to the end. Node "
    "shapes are string kinds; branches are coloured and `annotate`d yes/no/loop, on a clean "
    "paper theme with a colour legend."),
   ("complex-flowchart",
    "A big pipeline that builds itself, then runs: a 24-node CI/CD flow with 7 decisions and "
    "feedback loops auto-wraps into readable side-by-side columns (long loops routed around "
