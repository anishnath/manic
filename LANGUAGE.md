...
| `freekick(id, (cx,cy), speed, angle°, spin, [scale], [steps])` | a **spinning ball's curved flight** (the Magnus effect) — RK4 with drag + a Magnus side-force `κ·spin·(⊥v)`. Top-down: `angle` 0 = straight, + = right; `spin` signed sidespin (0 = straight shot, larger = more curl). Lays out the gold trajectory `{id}` (draw it) + `{id}.ball`, and registers a sim so `run(id, dur)` flies the ball along the banana. |
...
| `c4(id, [level])` | C4-model canvas (Simon Brown's Context → Container → Component). Reuses the architecture layout + scale-to-fit but lays out in the conventional **people-top tiers** (person / internal systems / external systems); a wide tier **auto-splits** into a balanced grid. `level` (`context`/`container`/`component`) is advisory. Elements are outline-styled (coloured border + text, transparent fill); a relationship that spans *upward* across tiers auto-routes around the margin so it never bisects the diagram. Zoom between levels is author-composed with `zoom`/`fade`/`show` |
...
| `node(id, parent, "kind", "name", [description], [technology])` | inside a `c4`, `kind` is a C4 element — `person`/`system`/`container`/`component`/`external`; the optional description and technology compose the standard label `name` / `[Type: tech]` / `description`. A `person` gets the classic head circle atop its box |
